...
const HELP: Record<string, HelpEntry> = {
  ...
  invoices: {
    ...
    steps: [
      ...
      { title: 'Odczyt AI (Beta) ✨', text: 'Prześlij PDF lub zdjęcie faktury, a następnie kliknij "Odczytaj fakturę AI". System automatycznie rozpozna wszystkie pozycje, ceny i dane nagłówkowe. Działa z wielostronicowymi fakturami PDF. Przed wstawieniem zawsze sprawdź dane — odczyt AI może nie być w 100% dokładny.' },
      { title: 'Podgląd AI — edycja przed wstawieniem', text: 'Po skanowaniu pojawia się okno podglądu z wszystkimi odczytanymi pozycjami. Możesz poprawić każdą linię, zmienić typ faktury (COS/SEMIS), dodać lub usunąć pozycje. Dopiero po kliknięciu "Wstaw" dane trafiają do formularza.' },
      { title: 'Pozycje faktury (COS) — cena netto/brutto', text: 'Przy każdej linii COS kliknij przycisk N/B aby przełączyć tryb wprowadzania ceny: N = netto, B = brutto. Przy trybie B system automatycznie przelicza cenę netto na podstawie stawki VAT.' },
      { title: 'Pozycje faktury (COS) — rabat', text: 'Przy każdej pozycji COS dostępna jest kolumna "Rab.%". Kliknij przycisk % / zł aby wybrać tryb rabatu: procentowy (np. 5 = odlicz 5% od ceny jednostkowej) lub kwotowy (np. 2.50 = odlicz 2.50 zł od ceny). Wartości Netto i Brutto są automatycznie przeliczane po uwzględnieniu rabatu.' },
      { title: 'Pozycje faktury (COS) — zaznaczanie zbiorcze', text: 'Zaznacz kilka pozycji checkboxami i użyj paska zbiorczego aby ustawić jednocześnie: stawkę VAT, kategorię COS, jednostkę miary lub tryb ceny (netto/brutto) dla wszystkich zaznaczonych linii.' },
      { title: 'Pozycje faktury (SEMIS) — autocomplete', text: 'Pole opisu pozycji SEMIS podpowiada automatycznie opisy z poprzednich faktur oraz listę popularnych opisów (np. Czynsz za lokal, Energia elektryczna). Wpisz kilka liter i wybierz z listy.' },
      { title: 'Pozycje faktury (SEMIS) — zaznaczanie zbiorcze', text: 'Zaznacz kilka pozycji SEMIS i użyj paska zbiorczego aby ustawić kategorię jednocześnie dla wszystkich zaznaczonych.' },
      ...
      { title: 'Historia — paginacja', text: 'Zakładka Historia pokazuje wszystkie przesłane faktury po 50 na stronie. Używaj przycisków "Poprzednia" / "Następna" aby przeglądać starsze faktury. Widoczna jest łączna liczba faktur i numer bieżącej strony.' },
    ],
  },
  inventory: {
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-[76px] md:bottom-6 right-4 md:right-6 z-40 w-12 h-12 rounded-full bg-[#2563EB] text-white shadow-lg shadow-blue-500/30 hover:bg-[#1D4ED8] transition-all hover:scale-105 flex items-center justify-center"
        title="Pomoc"
      >
